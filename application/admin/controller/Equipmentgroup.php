...
class Equipmentgroup extends Base
{
    //首页
    ...
    public function editInit()
    {
        ...
        $check = DB::name('equipment_default')->where(['group_id'=>$params['group_id']])->find();
        if(empty($check)){
            $data['group_id'] = $params['group_id'];
            Db::name('equipment_default')->insertGetId($data);
            Db::name('equipment')->where(['group_id'=>$params['group_id']])->update($data);
            return json(['status'=>1,'info'=>'操作成功']);
        }else {
            Db::name('equipment')->where(['group_id'=>$params['group_id']])->update($data);
            Db::name('equipment_default')->where(['group_id'=>$params['group_id']])->update($data);
            return json(['status'=>1,'info'=>'操作成功']);
        }
    }
    ...
    //获取选择的组烟槽设置
    public function get_group_equipment(){
        ...
        $group_equipment=Db::name('equipment_default')
            ->where(['group_id'=>$group_id])
            ->field('id,groove_good1,groove_good2,groove_good3,groove_good4,groove_good5,groove_good6,groove_good7')
            ->find();

        return json(['status'=>1,'info'=>'success','data'=>$group_equipment]);
    }
}