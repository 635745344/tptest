...
//
    ...
    public function lists($page=1,$limit=10)
    {
        ...
        $where['e.group_id'] = $this->group_id;
        $data=Db::name('equipment')->alias('e')
            ->join('sp_shop s',' s.id=e.shop_id ','left')
            ->join('sp_equipment_group g',' g.id=e.group_id ','left')
            ->where($where)
            ->field('e.id,g.group_name,e.code,e.groove1,e.groove2,e.groove3,e.groove4,e.groove5,e.groove6,e.groove7,e.status,s.province,s.city,s.district,s.address')
    ...
    public function monitorlist($eq_code='',$goods_name='',$page=1,$limit=10)
    {
        ...
        $data = Db::name('receive_record')->alias('rr')
            ...
            ->join('sp_equipment e','rr.eq_code=e.code')
            ->where($where)
            ->where($where2)
            ->page($page,$limit)
            ->order('rr.create_time desc')
            ->field('rr.id,rr.eq_code,u.nickname,rr.groove_num,g.name goods_name,rr.status,FROM_UNIXTIME(rr.create_time) create_time')
            ->select();
