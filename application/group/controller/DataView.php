...
class DataView extends Check
{
   ...
   {
       $group_list = Db::name('equipment_group')->where(['status'=>1,'id'=>$this->group_id])->field('id as group_id,group_name')->select();
    ...
    */
    ...
    public function smoke_hour_data()
    {
        ...
        $day = date('Y-m-d H:i:s',$start_day).'/'. date('Y-m-d H:i:s',$end_day);
        ...
        return json(['status'=>1,'msg'=>'查询成功','info'=>$data,'select_day'=>$day]);
    ...
    */
    ...
    public function goods_data()
    {
        ...
        if($this->new_view==1)
        {
            ...
            $goods_id_list = Db::name('v_goods_count_two')->where($where)->field('goods_id,goods_name')->group('goods_id')->select();

        }

//        $list = Db::name('v_goods_count')->where($where)->select();
        if($this->new_view ==1 ) $where2 =" and  (`rr`.`is_callblack` = 1) ";
        ...
        $sql = "select k.goods_id,k.goods_name from (select `rr`.`goods_id` AS `goods_id`,count(`rr`.`goods_id`) AS `goods_count`,
                ...
                where ((`rr`.`status` = 1)) $where2 and rr.`create_time` BETWEEN $start_day and $end_day
    ...
    /*
      ...
      * @return json
     */
    public function shop_coordinate()
    {
        ...
        foreach ($goods_id_list as $key => $value)
        {
            ...
            for ($i=0;$i<$r;$i++)
            {
                ...
                if($this->new_view==1){
                    ...
                    $count = Db::name('v_goods_count_two')->where(['create_day'=>$day,'goods_id'=>$goods_id])->value('goods_count');
                }
        ...
        foreach ($goods_id_list as $key => $value)
        {
            ...
            for ($i=0;$i<=9;$i++)
            {
                ...
                if($this->new_view==1){
                    ...
                    $count = Db::name('v_receive_record_two')->where($where)->count();
                }
