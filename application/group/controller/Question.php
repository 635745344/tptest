...
class Question extends Check
{
    ...
    public function addView()
    {
        ...
        if(!empty($id)){
            ...
            $question=Db::name('question')->where(['questionnaire_id'=>$id,'group_id'=>$this->group_id])->field('id,title,type,option')->select();
        }
    ...
    {
        $data = Db::name('questionnaire')->where(['group_id'=>$this->group_id])
    ...
    {
        Db::name('questionnaire')->where(['id'=>$id,'group_id'=>$this->group_id])->update(['status'=>1,'update_time'=>time()]);
