...
class ChuyanjiTest extends Base
{
...
//        $this->mem = MemcacheInstance::getInstance (); // 采用单例模式调用Memcached
    ...
    public function initEquipment()
    {
        ...
        if($check_equipment){ 
            ...
            Db::name('equipment')->where(['equipment_id'=>$equipment_id])->update(['jg_push_Id'=>$jg_push_id,'tx_push_Id'=>$tx_push_id,'mac'=>$mac]);
        ...
        P(['jg_push_id'=>$jg_push_id,'tx_push_id'=>$tx_push_id,'equipment_id'=>$equipment_id,'new_code'=>$new_code,'mac'=>$mac,'check_equipment'=>$check_equipment]);
    ...
    public function updateJgpushId()
    {
        ...
        DB::name('equipment')->where(['code'=>$eq_code])->update(['jg_push_id'=>$jg_push_id]);
