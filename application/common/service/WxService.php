...
//
    ...
    public function sendChuanjiUrl($eq_code, $touser)
    {
        ...
        $group_id = Db::name('equipment')->where(['code' => $eq_code])->value('group_id');
        ...
        if ($set['given'] == 1) {
            $parame = array(
                ...
                "msgtype" => "image",
                "image" => array(
                    "media_id" => "fG_k8n0tLn78AlxwWMrVsQ9bAVmiwWML9HsAcBVwWJI",
                )
            );
        } else {
                ...
                "news" => array(
                    "articles" => array(
                        array(
                            ...
                            "url" => $url,
                            "picurl" => 'https://img.zonma.net/logo.jpg',
                        ),
                    )
                )
            );
    ...
    //发送问卷调查模板
    public function sendAlarmTemplate($data)
    {
        $groove_good = $data['groove_good']; //剩余数量
        $goods_name = $data['goods_name']; //烟名字
        $alarm_count = $data['alarm_count']; //阀值
        $groove = $data['groove']; //烟槽
        $eq_code = $data['eq_code'];

        $time = date('Y-m-d H:i:s', time());
        $first = "编号：" . $eq_code . "出烟机烟仓库存数量警报";
        $join_count = "库存警报";
        $remark = "警报提示：编号[" . $eq_code . "]出烟机," . $groove . "号烟仓(" . $goods_name . "),库存数量低于" . $alarm_count . ",当前烟仓剩余数量:" . $groove_good;
        $list = [
            'first' => ['value' => $first, 'color' => '#173177'],
            'keyword1' => ['value' => $time, 'color' => '#173177'],  //标题
            'keyword2' => ['value' => $join_count, 'color' => '#173177'], //参与人数
            'remark' => ['value' => $remark, 'color' => '#173177']
        ];

        $data = [
            'touser' => $data['touser'],
            'template_id' => '9CBh2yT_X2jAtIFFbGboWZlrwwUzEBVVIMcaGkkDxnQ',
            'url' => '',
            'data' => $list
        ];
//
