...
var address = function (geturls, saveurls, success) {
    //获取个人信息请求地址
    var geturl = geturls;
    //保存个人信息请求地址
    var saveurl = saveurls;
        ...
        $.ajax({
            ...
            success: function (e) {
                ...
                if (e.status == 0) {
                    ...
                    $("#phone").val(e.info.phone);




                    $target.val(e.info.province + ' ' + e.info.city + ' ' + e.info.district);
                    $target.citySelect({
                        provance: e.info.province,
                        city: e.info.city,
                        area: e.info.district
                    });

                    $("#province").val(e.info.province);
                    $("#city").val(e.info.city);
                    $("#district").val(e.info.district);
                    $("#address").val(e.info.address);
    ...
    $("#saveuserinfo").click(function () {
        if ($.trim($("#name").val()) == "" || $.trim($("#phone").val()) == "" || $.trim($("#detailed").val()) == "" || $.trim($("#address").val()) == "") {
            alert("请输入完整信息");
            return;
        }
        var re = /^1\d{10}$/;
        if (re.test($("#phone").val()) == false) {
            alert("请输入正确的手机号");
            return;
        }
        var data = {};
        ...
        data.district = $.trim($("#district").val());
        console.log(data);
